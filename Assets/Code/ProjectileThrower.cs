...
public class ProjectileThrower : MonoBehaviour {
    ...
    {
		Rigidbody2D[] rigidObjects = Object.FindObjectsOfType<Rigidbody2D> ();
		foreach (Rigidbody2D r in rigidObjects) {
			if (IsActive (r))
				return true;
		}
		return false;
    ...
    private void FireControl()
    {
		...
		case FiringState.Firing:
			...
			if (!WaitingForPhysicsToSettle()) {
				ResetForFiring ();
			}
			break;
        }

		if (Input.GetButtonDown ("Cancel"))//reset level if user is pressing escape key
			ResetForFiring ();
    }
